...
class Index extends React.Component{
            ...
            <div className="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="false">
                    <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        {this.state.customerId != null?
                            <button onClick={() => this.updateModal(this.state.customerId)} type="button" className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium bg-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:w-auto sm:text-sm">
                            Updaed
                            </button> 
                            : 
    ...
    async saveModal() {
       console.log(this.state.name, this.state.email, this.state.phone, this.state.address, this.state.active)
       await CustomerDatas.addCustomer(this.state.name, this.state.email, this.state.phone, this.state.address, this.state.active)
    ...
    async editCustomer (id) {
        CustomerDatas.data.find((customer, index) =>{if(customer.id === id){
            this.setState({
                name: customer.name,
                                ...
                                <table className="bg-secondary w-width99% mx-auto my-1 overflow-y-auto">
                                   <thead className="bg-gray text-white uppercase">
                                    ...
                                    <tbody>
                                            ...
                                            {CustomerDatas != undefined ? CustomerDatas.data.map( (list) => 
                                            ...
                                            <td className="border-white border flex items-center justify-center">
                                                <button onClick={() =>this.editCustomer(list.id)} className="text-primary hover:text-blue">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                    </svg>
                                                </button>
                                                <button onClick={() => this.deleteCustomer(list.id, list)} className="text-red hover:text-red-300">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
